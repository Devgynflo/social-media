import { Metadata, NextPage } from 'next'
import Image from 'next/image'

import signupImg from '@/assets/signup-image.jpg'
import Link from 'next/link'
import { SignUpForm } from './_components/sign-up-form'

export const metadata: Metadata = {
    title: 'Signup',
}

interface SignupPageProps {}

const SignupPage: NextPage<SignupPageProps> = ({}) => {
  return <div className='flex h-screen items-center justify-center p-5'>
    <div className="flex h-full max-h-[40rem] w-full max-w-[64rem] rounded-2xl overflow-hidden bg-card shadow-2xl">
        <div className='md:w-1/2 w-full space-y-10 overflow-y-auto p-10'>
            <div className="space-y-1 text-center">
                <h1 className='tex-3xl font-bold'>Inscription Social Media</h1>
                <p className="text-muted-foreground">
                    Inscrivez-vous pour <span className='italic'>profiter</span> de toutes les fonctionnalités de notre site.
                </p>
            </div>
            <div className="space-y-5">
                <SignUpForm />
            </div>
            <Link href={'/login'} className='block text-center hover:underline'>Déjà inscrit ? Connectez-vous</Link>
        </div>
        <Image src={signupImg} alt="signup" className='w-1/2 hidden md:block object-cover' />
    </div>
  </div>
}

export default SignupPage